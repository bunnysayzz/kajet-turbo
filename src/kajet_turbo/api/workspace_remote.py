...
def _guard(user: CurrentUser, name: str, ws_service: WorkspaceService) -> JSONResponse | None:
    """Return None on success, or a 403 JSONResponse when the user lacks workspace access.
    Only for sync `def` routes -- FastAPI already runs those off the event loop, so the
    direct `has_access` call is not a blocking-I/O bug here. Async routes use
    `_guard_async` instead."""
...
async def _guard_async(
    user: CurrentUser, name: str, ws_service: WorkspaceService
) -> JSONResponse | None:
...
async def api_set_workspace_remote(
    ...
    if deny := await _guard_async(user, name, ws_service):
        return deny
    try:
