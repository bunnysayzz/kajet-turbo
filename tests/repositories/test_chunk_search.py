...
@pytest.mark.parametrize(
    ...
    [
        ...
        # A hyphenated numeric was read as a column reference ("no such column: latki").
        ("18-year-olds", '"year" OR "olds"'),
        ...
        # Non-ASCII is the norm for this corpus, so it must survive tokenisation.
        ("zażółć gęślą", '"zażółć" OR "gęślą"'),
        # Shorter than a trigram: unmatchable, and poisons an AND chain. Dropped.
        ("Julia z Kalisza", '"Julia" OR "Kalisza"'),
        ("a b c", ""),
...
@pytest.mark.parametrize(
    "query",
    [
        "zatrzymanie się przed intensywnością, blokada przed doświadczeniem",
        "molo promenada telefon hiszpania 18-latki rozmowa",
        "Rozłączono — koncept książki, party girl, spisek",
...
def test_search_fts_survives_queries_that_used_to_raise(database, query):
    """Each of these came from a production `search_fts_failed` warning."""
    repo = _seed(database)
